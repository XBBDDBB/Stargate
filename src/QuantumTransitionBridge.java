...
public class QuantumTransitionBridge {
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        LogUtils.recordINFOLog("参数识别完毕。本次需要同步的表共有"+tableArray.length+"个。正在初始化任务组信息。。。");
        ...
        LogUtils.recordINFOLog("分配「数据拆分」任务成功，线程池已启动，开始进入包工头模式。。。");
        Object result;
        ...
        do {
            ...
            for(Future future : futureList){
                if(future.isDone()){
                    LogUtils.recordINFOLog("好耶，有一个人跟我说他干完了！让我看看。。。");
                    result = future.get();
                    if("$O$K$".equals(result)){
                        LogUtils.recordINFOLog("好耶干的不错！");
                    }else if(result!=null){
                        LogUtils.recordERRORLog("哦天啊他炸了。信息如下："+result.toString().replace("$O$K$",""));
            ...
            finishList.clear();
            LogUtils.recordINFOLog("检查线程池状态，目前还有"+futureList.size()+"个人没有干完活。");
        }while(futureList.size()>0);
        poolExecutors.shutdown();
        LogUtils.recordINFOLog("总计"+tableArray.length+"个表需要同步，本次一共"+ParamUtils.paramMap.get("tablePoolSize")+"只牛马给我干活，「数据拆分」任务全部执行完毕,共有"+errorJobList.size()+"个人爆炸。");
        for(int i=0;i<errorJobList.size();i++){
            if(i==0){
                LogUtils.recordERRORLog("为了防止前面的错误信息刷没，我再统一告诉你一次吧。");
            }
            LogUtils.recordERRORLog("第"+(i+1)+"个工人跟我说："+errorJobList.get(i).toString());
        }
        if(errorJobList.size()>0){
            if("1".equals(ParamUtils.paramMap.get("errorHandingType"))){
                LogUtils.recordERRORLog("程序遇到严重错误，直接中断程序吧");
                //遇到严重错误，直接中断程序吧
                System.exit(1);
            }
        }
        String errmsg = errorJobList.size()>0?"有报错":"无报错";
        LogUtils.recordINFOLog("数据拆分任务执行完毕！"+errmsg+"，当前任务队列共有"+ParamUtils.queue.size()+"个任务待消费，开始创建线程池。。。");

        //第二次用线程池，为了数据同步，消费任务队列
        //创建固定长度的线程组
        poolExecutors = Executors.newFixedThreadPool(Integer.parseInt(ParamUtils.paramMap.get("dataPoolSize")));
        LogUtils.recordINFOLog("线程池创建完毕！开始分配「数据同步」任务。。。");
        //用来接任务组线程的返回值
        futureList = new ArrayList<Future>();
        for(int i=0;i<Integer.parseInt(ParamUtils.paramMap.get("dataPoolSize"));i++){
            Future future = poolExecutors.submit(new SynThread(ParamUtils.paramMap,i));
            futureList.add(future);
        }
        LogUtils.recordINFOLog("分配「数据同步」任务成功，线程池已启动，开始进入包工头模式。。。");
        result = null;
        ...
        poolExecutors.shutdown();
        LogUtils.recordINFOLog("总计"+tableArray.length+"个表需要同步，本次一共"+ParamUtils.paramMap.get("dataPoolSize")+"只牛马给我干活，「数据同步」任务全部执行完毕,共有"+errorJobList.size()+"个人爆炸。");
        for(int i=0;i<errorJobList.size();i++){
            if(i==0){
                LogUtils.recordERRORLog("为了防止前面的错误信息刷没，我再统一告诉你一次吧。");
            }
